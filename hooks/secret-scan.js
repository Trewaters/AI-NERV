...
process.stdin.on("end", () => {
  ...
  // Valid JSON that isn't an object (null, an array, a bare string) is still
  // not a hook payload — tolerate it rather than throwing past the handler.
  if (!data || typeof data !== "object") process.exit(0);
  const toolInput = data.tool_input || {};
  const filePath = toolInput.file_path || "";
  if (ALLOWED_FILE.test(filePath)) process.exit(0);
  ...
  const [found] = findSecrets(newContent, oldContent);
  if (found) {
    console.error(
      `Blocked: this change to ${filePath || "this file"} would introduce a ${found.name} (${found.match.slice(0, 12)}...). ` +
